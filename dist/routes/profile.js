...
router.get('/', auth_1.requireAuth, async (req, res) => {
    try {
        const { rows } = await db_1.pool.query('SELECT * FROM profiles WHERE id = $1', [req.user.id]);
        if (!rows[0]) {
...
router.patch('/', auth_1.requireAuth, async (req, res) => {
    ...
    try {
        const { rows } = await db_1.pool.query(`UPDATE profiles SET ${setClauses} WHERE id = $1 RETURNING *`, values);
        res.json(rows[0]);
    }
    catch (err) {
...
router.get('/:userId', auth_1.requireAuth, async (req, res) => {
    ...
    try {
        const { rows } = await db_1.pool.query('SELECT * FROM profiles WHERE id = $1', [userId]);
        if (!rows[0]) {
            res.status(404).json({ error: 'Profile not found' });
            return;
        }
