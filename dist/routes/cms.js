...
router.get('/', async (req, res) => {
    ...
    try {
        let q = db_1.pool.query.bind(db_1.pool);
        let sql = 'SELECT * FROM cms_content WHERE 1=1';
        const params = [];
        if (page) {
            params.push(page);
            sql += ` AND key LIKE $${params.length} || ':%'`;
        }
        // Simple key-value store: key is "page::section::fieldname"
        const fullKey = [page, section, key].filter(Boolean).join('::');
        if (fullKey) {
            const { rows } = await db_1.pool.query('SELECT value FROM cms_content WHERE key = $1', [fullKey]);
            res.json(rows[0]?.value ?? null);
            return;
        }
        const { rows } = await db_1.pool.query('SELECT key, value FROM cms_content');
        res.json(rows);
    }
    catch (err) {
...
router.put('/:key(*)', auth_1.requireAuth, (0, auth_1.requireRole)('admin'), async (req, res) => {
    ...
    try {
        const { rows } = await db_1.pool.query(`INSERT INTO cms_content (key, value, updated_by) VALUES ($1, $2, $3)
       ON CONFLICT (key) DO UPDATE SET value = $2, updated_by = $3, updated_at = NOW()
       RETURNING *`, [key, JSON.stringify(value), req.user.id]);
        res.json(rows[0]);
    }
    catch (err) {
...
router.get('/seo/:pageName', async (req, res) => {
    try {
        const { rows } = await db_1.pool.query("SELECT value FROM cms_content WHERE key = $1", [`seo::${req.params.pageName}`]);
        res.json(rows[0]?.value ?? null);
    }
    catch (err) {
...
router.get('/traffic-messages', async (_req, res) => {
    try {
        const { rows } = await db_1.pool.query('SELECT * FROM traffic_messages WHERE active = true ORDER BY created_at DESC');
        res.json(rows);
    }
    catch (err) {
...
router.post('/traffic-messages', auth_1.requireAuth, (0, auth_1.requireRole)('admin'), async (req, res) => {
    ...
    try {
        const { rows } = await db_1.pool.query('INSERT INTO traffic_messages (message, created_by) VALUES ($1, $2) RETURNING *', [message, req.user.id]);
