...
const uploadsDir = path_1.default.join(__dirname, '../../uploads/driver-docs');
...
router.get('/profile', auth_1.requireAuth, async (req, res) => {
    try {
        const { rows } = await db_1.pool.query('SELECT * FROM driver_profiles WHERE user_id = $1', [req.user.id]);
...
router.patch('/profile', auth_1.requireAuth, async (req, res) => {
    ...
    try {
        const { rows } = await db_1.pool.query(`INSERT INTO driver_profiles (user_id, ${Object.keys(updates).join(', ')})
...
router.get('/application', auth_1.requireAuth, async (req, res) => {
    try {
        const { rows } = await db_1.pool.query('SELECT * FROM driver_applications WHERE user_id = $1 ORDER BY created_at DESC LIMIT 1', [req.user.id]);
        res.json(rows[0] || null);
    }
    catch (err) {
...
router.post('/application', auth_1.requireAuth, upload.fields(docFields), async (req, res) => {
    const files = req.files;
    const baseUrl = process.env.BASE_URL || 'http://localhost:3000';
    const docUrl = (fieldName) => files?.[fieldName]?.[0]
        ? `${baseUrl}/uploads/driver-docs/${files[fieldName][0].filename}`
        : null;
    const { full_name, phone, vehicle_type, vehicle_color, plate_number, years_experience, languages_spoken, bank_name, bank_account_number, bank_account_holder, date_of_birth, national_id_number, address, city, vehicle_brand, vehicle_model, vehicle_year, } = req.body;
    try {
        const { rows } = await db_1.pool.query(`INSERT INTO driver_applications
        ...
        const upsertDoc = async (driverId, docType, url) => {
            await db_1.pool.query(`INSERT INTO driver_documents (driver_id, document_type, file_url, status, uploaded_at)
           VALUES ($1, $2, $3, 'pending', NOW())
           ON CONFLICT (driver_id, document_type) DO UPDATE SET
             file_url = EXCLUDED.file_url, status = 'pending', uploaded_at = NOW()`, [driverId, docType, url]).catch((e) => console.error(`driver_documents upsert [${docType}]:`, e.message));
        };
        ...
        if (files?.avatar?.[0]) {
            ...
            await db_1.pool.query('UPDATE profiles SET avatar_url = $1 WHERE id = $2', [avatarUrl, req.user.id])
...
router.patch('/application/:id', auth_1.requireAuth, upload.fields(docFields), async (req, res) => {
    const { id } = req.params;
    const files = req.files;
    const baseUrl = process.env.BASE_URL || 'http://localhost:3000';
    // Verify ownership
    const { rows: existing } = await db_1.pool.query('SELECT * FROM driver_applications WHERE id = $1 AND user_id = $2', [id, req.user.id]);
    if (!existing.length) {
        res.status(404).json({ error: 'Application not found' });
        return;
    }
    const app = existing[0];
    const docUrl = (fieldName, existingUrl) => files?.[fieldName]?.[0]
        ? `${baseUrl}/uploads/driver-docs/${files[fieldName][0].filename}`
        : existingUrl;
    const { full_name, phone, vehicle_type, vehicle_color, plate_number, years_experience, languages_spoken, bank_name, bank_account_number, bank_account_holder, date_of_birth, national_id_number, address, city, vehicle_brand, vehicle_model, vehicle_year, } = req.body;
    try {
        const { rows } = await db_1.pool.query(`UPDATE driver_applications SET
