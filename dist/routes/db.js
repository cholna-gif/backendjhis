...
const safeCol = (s) => COL_RE.test(s);
function buildWhere(query, paramOffset = 0) {
    const conditions = [];
    const values = [];
...
router.get('/:table', async (req, res) => {
    ...
    const { select = '*', order_by, limit, offset, ...filters } = req.query;
...
router.post('/:table', async (req, res) => {
    ...
    if (cols.length === 0) {
        res.status(400).json({ error: 'No valid columns' });
        return;
    }
    ...
    try {
        ...
        const { rows: inserted } = await db_1.pool.query(q, allValues);
...
router.post('/:table/upsert', async (req, res) => {
    ...
    try {
      ...
      RETURNING *`;
        const { rows } = await db_1.pool.query(q, allValues);
...
router.patch('/:table', async (req, res) => {
    ...
    const { sql: whereSql, values: whereValues } = buildWhere(req.query, updateCols.length);
    if (!whereSql) {
        res.status(400).json({ error: 'At least one filter required for update' });
        return;
    }
    try {
        const q = `UPDATE ${table} SET ${setClauses} ${whereSql} RETURNING *`;
        const { rows } = await db_1.pool.query(q, [...updateValues, ...whereValues]);
        ...
        if (table === 'driver_applications' && payload.status === 'approved' && rows.length > 0) {
         ...
         ON CONFLICT (user_id) DO UPDATE SET
           ...
           is_id_verified=true, is_license_verified=true, is_vehicle_verified=true, is_active=true`, [app.user_id, app.vehicle_type, app.plate_number, app.vehicle_color, app.vehicle_brand, app.vehicle_model]).catch(() => { });
            await db_1.pool.query(`INSERT INTO user_roles (user_id, role) VALUES ($1, 'driver') ON CONFLICT (user_id, role) DO NOTHING`, [app.user_id]).catch(() => { });
        }
        res.json(rows);
    }
    catch (err) {
...
router.delete('/:table', async (req, res) => {
    ...
    if (!whereSql) {
        res.status(400).json({ error: 'At least one filter required for delete' });
        return;
    }
    try {
