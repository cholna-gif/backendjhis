...
router.post('/log_admin_action', requireAuth, requireRole('admin'), async (req: AuthRequest, res: Response): Promise<void> => {
...
router.post('/admin_adjust_wallet', requireAuth, requireRole('admin'), async (req: AuthRequest, res: Response): Promise<void> => {
...
router.post('/send_driver_email', requireAuth, requireRole('admin'), async (req: AuthRequest, res: Response): Promise<void> => {
...
router.post('/handle_passenger_cancellation', requireAuth, async (req: AuthRequest, res: Response): Promise<void> => {
  const { p_ride_id } = req.body;
  if (!p_ride_id) { res.status(400).json({ error: 'p_ride_id required' }); return; }
  ...
  try {
    ...
    const ride = rows[0];
    if (!ride) { await client.query('ROLLBACK'); res.status(404).json({ error: 'Ride not found' }); return; }
    if (ride.passenger_id !== req.user!.id) { await client.query('ROLLBACK'); res.status(403).json({ error: 'Forbidden' }); return; }
    ...
    await client.query(
      ...
      [`Passenger cancelled (${tier})`, p_ride_id],
    );
    ...
    if (ride.payment_method === 'wallet' && ride.payment_status === 'paid') {
      ...
      if (penalty > 0) {
        ...
        await client.query(
          `INSERT INTO wallet_transactions (user_id, amount, reason, type) VALUES ($1,$2,$3,'debit')`,
          [req.user!.id, -penalty, `Cancellation fee — ride ${p_ride_id}`],
        );
      }
      if (refund > 0) {
        await client.query(
        ...
        await client.query(
          `INSERT INTO wallet_transactions (user_id, amount, reason, type) VALUES ($1,$2,$3,'credit')`,
          [req.user!.id, refund, `Refund for cancelled ride ${p_ride_id}`],
        );
      }
    }
...
router.post('/add_wallet_balance', requireAuth, async (req: AuthRequest, res: Response): Promise<void> => {
  const { p_amount, p_method } = req.body;
  if (!p_amount || p_amount <= 0) { res.status(400).json({ error: 'Valid p_amount required' }); return; }

  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    const { rows } = await client.query(
      `UPDATE profiles SET wallet_balance = wallet_balance + $1 WHERE id = $2 RETURNING wallet_balance`,
      [p_amount, req.user!.id],
    );
    await client.query(
      `INSERT INTO wallet_transactions (user_id, amount, reason, type) VALUES ($1,$2,$3,'credit')`,
      [req.user!.id, p_amount, `Top-up via ${p_method || 'unknown'}`],
    );
    await client.query('COMMIT');
...
router.post('/handle_driver_cancellation', requireAuth, async (req: AuthRequest, res: Response): Promise<void> => {
  const { p_ride_id } = req.body;
  if (!p_ride_id) { res.status(400).json({ error: 'p_ride_id required' }); return; }
  ...
  try {
    ...
    const ride = rows[0];
    if (!ride) { await client.query('ROLLBACK'); res.status(404).json({ error: 'Ride not found' }); return; }
    if (ride.driver_id !== req.user!.id) { await client.query('ROLLBACK'); res.status(403).json({ error: 'Forbidden' }); return; }
    ...
    if (requeue) {
      await client.query(
        `UPDATE rides SET status = 'pending', driver_id = NULL, matched_at = NULL,
         arrived_at = NULL, cancelled_by = NULL WHERE id = $1`,
        [p_ride_id],
      );
    } else {
      await client.query(
        `UPDATE rides SET status = 'cancelled', cancelled_at = NOW(), cancelled_by = 'driver',
         cancellation_reason = 'Driver cancelled' WHERE id = $1`,
        [p_ride_id],
      );
    }
